...
%% pLaTeX2e <1996/03/05> では \em で和文が強調されないようなので次を定義する。
%% \renewcommand{\em}{\rmfamily\itshape\gtfamily}
...
%% pLaTeX2e <1996/03/05> では \meta の < と > の中が空きすぎるので変更する。
%% \newcommand{\meta}[1]{{\noindent\hbox{\rm$\langle$#1$\rangle$}}}
\newcommand{\meta}[1]{{\noindent\hbox{\tt<\rm#1\tt>}}}
\let\hyper=\meta
...
\newenvironment{scheme}{
  ...
  \newcommand{\ev}{\>\>\evalsto}
  \newcommand{\lev}{\\\>\evalsto}
%% pLaTeX2e <1996/03/05> で \em で和文を強調できるようにするために変更する。
  \renewcommand{\em}{\rmfamily\itshape}
%%  \renewcommand{\em}{\rmfamily\itshape\gtfamily}
  \newcommand{\unspecified}{{\em{}未規定}}
...
\clearpage
\@topnewpage[
    \centerline{\large\bf\uppercase{概念の定義，キーワード，および手続きの}}
    \centerline{\large\bf\uppercase{アルファベット順索引}}
    \vskip 1ex \bigskip]
    \markboth{索引}{索引}
    \phantomsection
    \addcontentsline{toc}{chapter}{概念の定義，キーワード，および手続きの索引}
    \bgroup %\small
    \parindent\z@
...
%% pdf のしおり を作る
\usepackage[dvipdfm,%
bookmarks=true,bookmarksnumbered=false,bookmarkstype=toc,%
%colorlinks=true,% 
%colorlinks=false,%
%linkcolor=blue,%
%citecolor=blue,%
